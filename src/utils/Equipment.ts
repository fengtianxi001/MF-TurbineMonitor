...
class Equipment {
  ...
  async loadEquipment() {
    const glb = await this.threeInstance.loadGLTF("equipment.glb");
  ...
  equipmentClick(event: MouseEvent) {
    ...
    raycaster.setFromCamera(mouse, this.threeInstance.camera);
  ...
  outline(selectedObjects: THREE.Object3D<THREE.Event>[], color = 0x15c5e8) {
    const { renderer, camera, scene } = this.threeInstance;
    const [w, h] = [window.innerWidth, window.innerHeight];
    ...
    this.threeInstance.addCompose(compose);
  }
}
