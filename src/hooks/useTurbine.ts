...
export function useTurbine(element: Ref<HTMLElement>) {
  ...
  const { scene, camera, control, renderMixins, renderer, composers, loadGLTF, loadAnimate } = useThree(element)
  ...
  const _loadTurbineSkeleton = async () => {
    const { scene: object, animations } = await loadGLTF('turbine.glb', (percent) => (percents.skeleton = percent))
  ...
  const _updateYawAngle = () => {
    ...
    const _tweenAnimation = (oldValue: any, newValue: any, during: number, updateCallback: (data: any) => void) => {
  ...
  const _loadTurbineEquipments = async () => {
    const { scene: object } = await loadGLTF('equipment.glb', (percent) => (percents.equipment = percent))
  ...
  const _outline = (selected: THREE.Object3D<THREE.Event>[], color = 0x15c5e8) => {
